...
<svelte:head>
	<title>New Workflow Instance - Boilerworks</title>
</svelte:head>

<div class="mx-auto max-w-2xl">
	<h1 class="mb-6 text-2xl font-bold">Start New Workflow Instance</h1>
	...
	<form method="POST" class="bg-bw-surface border-bw-border space-y-4 rounded-lg border p-6">
		<div>
			<label for="definition_id" class="text-bw-text-muted mb-1 block text-sm">Workflow Definition</label>
			<select name="definition_id" id="definition_id" required
		...
		<div class="flex gap-3">
			...
			<a href="/workflows" class="text-bw-text-muted hover:text-bw-text rounded px-4 py-2 text-sm">Cancel</a>
		</div>
	</form>
</div>
