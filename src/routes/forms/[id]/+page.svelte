...
<div class="mb-6 flex items-center justify-between">
	...
	<div class="flex gap-3">
		<a
			href="/forms/{data.form.id}/submit"
			...
			{#each data.submissions as sub}
				<tr class="border-bw-border hover:bg-bw-surface-hover border-b last:border-0">
					...
					<td class="text-bw-text-muted px-4 py-3 text-sm"
						>{new Date(sub.createdAt).toLocaleString()}</td
					>
				</tr>
			{:else}
				<tr>
...
<a href="/forms" class="text-bw-text-muted hover:text-bw-text mt-4 inline-block text-sm"
	>&larr; Back to Forms</a
>
