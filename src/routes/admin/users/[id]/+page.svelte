...
<div class="mx-auto max-w-2xl">
	...
	<form
		method="POST"
		action="?/update"
		class="bg-bw-surface border-bw-border space-y-4 rounded-lg border p-6"
	>
		<div>
			...
			<input
				...
				id="display_name"
				required
				value={data.targetUser.displayName}
		...
		<div class="flex items-center gap-4">
			<div class="flex items-center gap-2">
				<input
					...
					name="is_active"
					id="is_active"
					checked={data.targetUser.isActive}
					class="rounded"
				/>
			...
			<div class="flex items-center gap-2">
				<input
					...
					name="is_superuser"
					id="is_superuser"
					checked={data.targetUser.isSuperuser}
					class="rounded"
				/>
				...
				{#each data.groups as group}
					<div class="flex items-center gap-2">
						<input
							...
							name="group_ids"
							value={group.id}
							checked={data.memberGroupIds.includes(group.id)}
							class="rounded"
						/>
		...
		<div class="flex gap-3">
			...
			<a href="/admin/users" class="text-bw-text-muted hover:text-bw-text rounded px-4 py-2 text-sm"
				>Cancel</a
			>
		</div>
	</form>
	...
	<form method="POST" action="?/deactivate" class="mt-6">
		<button
			...
			onclick={(e) => {
				if (!confirm('Are you sure you want to deactivate this user?')) e.preventDefault();
			}}
		>
			Deactivate User
		</button>
