...
<svelte:head>
	<title>Products - Boilerworks</title>
</svelte:head>
		...
		<tbody>
			{#each data.products as product}
				<tr class="border-bw-border hover:bg-bw-surface-hover border-b last:border-0">
					...
					<td class="text-bw-text-muted px-4 py-3 text-sm">{product.categoryName ?? '—'}</td>
					...
					<td class="px-4 py-3 text-right text-sm">
						<a href="/products/{product.id}" class="text-bw-accent hover:underline">View</a>
						<a href="/products/{product.id}/edit" class="text-bw-accent hover:underline ml-3">Edit</a>
					</td>
				</tr>
			{:else}
				<tr>
					<td colspan="5" class="text-bw-text-muted px-4 py-8 text-center text-sm">No products yet</td>
				</tr>
			{/each}
		</tbody>
	</table>
