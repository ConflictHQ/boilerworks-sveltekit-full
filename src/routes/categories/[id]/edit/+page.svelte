...
<svelte:head>
	<title>Edit {data.category.name} - Boilerworks</title>
</svelte:head>
...
<div class="mx-auto max-w-2xl">
	...
	<form method="POST" class="bg-bw-surface border-bw-border space-y-4 rounded-lg border p-6">
		<div>
			...
			<input type="text" name="name" id="name" required value={data.category.name}
		...
		<div>
			...
			<input type="text" name="slug" id="slug" required value={data.category.slug}
				class="bg-bw-bg border-bw-border text-bw-text focus:border-bw-accent w-full rounded border px-3 py-2 focus:outline-none" />
		</div>
		...
		<div class="flex gap-3">
			...
			<a href="/categories/{data.category.id}" class="text-bw-text-muted hover:text-bw-text rounded px-4 py-2 text-sm">Cancel</a>
		</div>
	</form>
</div>
